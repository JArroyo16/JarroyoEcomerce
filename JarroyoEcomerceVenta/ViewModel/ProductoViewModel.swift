...
class ProductoViewModel{
    ...
    static func Update (producto: Producto) -> Result{
        ...
        do {
            if try(sqlite3_prepare_v2(context.db, query, -1, &statement, nil) == SQLITE_OK){
                ...
                sqlite3_bind_int64(statement, 2, sqlite3_int64((producto.PrecioUnitario) as! NSNumber))
                sqlite3_bind_int64(statement, 3, sqlite3_int64((producto.Stock) as! NSNumber))
                //sqlite3_bind_int64(statement, 4, sqlite3_int64((producto.Area?.IdArea) as! NSNumber))
    ...
    static func GetByNombre(Nombre : String) -> Result{
        var result = Result()
        var context = DBManger()
        var statement : OpaquePointer?
        var query = "SELECT Imagen, Nombre, Descripcion, PrecioUnitario FROM Producto WHERE Nombre LIKE '%\(Nombre)%'"
        ...
        do{
            if try sqlite3_prepare_v2(context.db, query, -1, &statement, nil) == SQLITE_OK{
                ...
                //if try sqlite3_step(statement) == SQLITE_ROW{
                    while try sqlite3_step(statement) == SQLITE_ROW{
                        var producto = Producto()
                        producto.Imagen = String(describing: String(cString: sqlite3_column_text(statement, 0)))
                        producto.Nombre = String(describing: String(cString: sqlite3_column_text(statement, 1)))
                        producto.Descripcion = String(describing: String(cString: sqlite3_column_text(statement, 2)))
                        producto.PrecioUnitario = Int(sqlite3_column_int(statement,3))
    ...
    static func GetByIdDep(IdDepartamento : Int) -> Result{
        var context = DBManger()
        var result = Result()
        var query = "SELECT Imagen, Nombre, Descripcion, PrecioUnitario FROM Producto WHERE IdDepartamento = \(IdDepartamento)"
        var statement : OpaquePointer?
